...
//error_reporting(E_ALL);
	...
	$to_mobile=$_COOKIE['chatting_with'];
	if (empty($to_mobile)) {
	...
	$sql="INSERT INTO `dt_pics` (
			...
			$c->insert($sql);
			$c->sms($to_mobile,"Anon","http://txt.am/w/?$fname");
			file_get_contents("http://gaysugardaddyfinder.com/inc/x_send_api.php?to=".$_COOKIE[chatting_with]."&from=anon&msg="."http://txt.am/w/?$fname");
			$c->insert("update sms_sessions set data='$fname|$ext' where mobile='$from_mobile'");
			mailme("$to_mobile," . $_COOKIE['mobile'] . "http://txt.am/w/?$fname");
		}
