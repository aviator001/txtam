...
<?
		...
		if (($ext=='mp4')||($ext=='mpg')||($ext=='mpeg')||($ext=='avi')) {
			...
			$strImg="<video loop id=\"bgvid\">
							<source src=\"../s/$qs.$ext\" type=\"video/mp4\">
						</video>";
		} else if (($ext=='png')||($ext=='jpg')||($ext=='gif')||($ext=='jpeg')) {
			$el="g_img";
			$strImg="<img style=\"position:absolute;opacity:0;width:30%;min-width:300px;margin:auto;left:0;right:0;top:80px;border:10px solid white;\" class=\"www_box\" id=\"g_img\" src='../s/$qs.$ext'>";
		} else {
			$strImg="<audio></audio>";
		}
		echo $strImg;
		echo "<img id=\"iplay\" src=\"http://sugardaddydays.com/assets2/grey_button.png\" style='border:none;position:fixed;margin:auto;left:0;right:0;bottom:5px;width:120px;height:120px'>";
		$ip=$c->getIP();
		$dt=date("Y-m-d H:i:s");
		...
		echo "<div id='timer'></div><a href=''><img id='banner' style='position:absolute;z-index:9;left:0;right:0;top:0;bottom:0;margin:auto;width:200px;' src='http://sugardaddydays.com/assets/images/no_images.png'></a>";
	}
?> 
...
<? } ?>
	...
	function touchStart(event){
		...
		$('#bgvid').click()
		media.play();
		el.src='../assets2/grey_button_o.png'
		if (timerStarted=='No') {
			timerStarted='Yes'
	...
	function touchPEnd(event){
		...
		if (!release) {
			...
			el.src='../assets2/grey_button.png'
		...
		if (total_seconds==0) {
			...
			if ((total_minutes==0)) {
				...
				if ((total_hours==0)) {
					...
					media.src='../assets/images/snow.gif'
