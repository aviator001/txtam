...
class utils {
	...
	public function __construct ($ip='') {
		...
		$f="/home/sugardaddydays/settings";
		try {
				$iniSet = fopen($f, "r");
				while(!feof($iniSet)) {
					$xln=fgets($iniSet);
				...
				$this->secrets=$arr;
				$this->logins=$Server;
				$this->host="http://sugardaddydays.com";
	...
	public function print_blocks($obj) {
		$obj=str_replace(' ','',$obj);
		$a=array('[',']','(',')','.','-');
		$b=array('','','','','','');
		str_replace($aa,$b,$obj);
		for ($i=0;$i<=strlen($obj)-1;$i++) {
			$str .= "<img onerror=\"this.src='http://txt.am/assets/img/space.png'\" src=\"assets/img/" . substr($obj,$i,1) . ".png\" style=\"width:12px\">";
		}
		return $str;
	}		
	...
	public function getNewNumber($from_real_number) {
		$from_real_number = $this->isValidMobile($from);
		$q = $this->query("select long_code from sms_long_codes where mobile = '$from_real_number'");
		if (empty($q[0][long_code])) {
			$from_long_code=$this->query("select long_code from sms_long_codes where mobile='' and long_code not in (select long_code from sms_tracker) order by id asc limit 1")[0]['long_code'];		
			$this->insert("UPDATE `sms_long_codes` SET `mobile`='$from_real_number', `active`='1' WHERE (`long_code`='$from_long_code') AND (`mobile`='')");
		} else {
			$from_long_code=$q[0][long_code];
		}
		return $from_long_code;
	}
	
	public function connect($ipx="199.91.65.82",$dbx="sddays") {
			global $db;
			try {
				global $conn;
//				$db = new mysqli("199.91.65.85", "gautamadmin", "Shadow2015!", "gaysugardaddyforme");
	...
	public function getLatLng() {
		$latLng=file_get_contents("http://terrawire.com:8080/json/".$this->getIP());
	...
	public function adjustImage($filename1,$mode=0, $annotate="No Text Given!") {
		...
		$filename_p="/home/gaysugar/public_html/photos/" . rand(1111,9999) . $filename1;
		...
		switch ( $mode ) {
			...
			case "RESET":
						...
						return 'photos/' . explode('/',$filename)[(count(explode('/',$filename))-1)];
			}
		
		$img->writeImage($filename_p);
		$img->writeImage($filename);
		imagedestroy($im);
		return 'photos/' . explode('/',$filename_p)[(count(explode('/',$filename_p))-1)];
	}
