...
//				$db = new mysqli("199.91.65.85", "gautamadmin", "Shadow2015!", "gaysugardaddyforme");
	...
	public function adjustImage($filename1,$mode=0, $annotate="No Text Given!") {
		...
		$instagraph->setInput("../snappy/".$filename1);
		$instagraph->setOutput("../snappy/".$filename1);
		require("/home/txt/public_html/assets/wi/WideImage.php");
		$filename="/home/txt/public_html/snappy/" . $filename1;
		$filename_p="/home/txt/public_html/snappy/" . rand(1111,9999) . $filename1;
		$filename_o="/home/txt/public_html/snappy/original_" . $filename1;
		$filename_r="i/" . $filename1; 
		$im=WideImage::load($filename);
		if (!getimagesize("/home/txt/public_html/snappy/original_" . $filename1)) $im->saveToFile($filename_o);
		switch ( $mode ) {
			case "CCW":
			...
			case "NORMALIZE":
						...
						$img->normalizeImage();
						//$img->compositeimage($original, \Imagick::COMPOSITE_ATOP, 0, 0);
						break;
			case "ANNOTATE":
			...
			case "RESET":
						...
						break;
			case "TOAST":
				$instagraph->process("Toaster"); // name of the filter
				return 'snappy/' . $filename1;
			case "NASHVILLE":
				$instagraph->process("Nashville"); // name of the filter
				return 'snappy/' . $filename1;
			case "LOMO":
				$instagraph->process("Lomo"); // name of the filter
				return 'snappy/' . $filename1;
			case "KELVIN":
				$instagraph->process("Kelvin"); // name of the filter
				return 'snappy/' . $filename1;			
			case "GOTHAM":
				$instagraph->process("Gotham"); // name of the filter
				return 'snappy/' . $filename1;
		   }
	...
	public function sms($to, $from, $message) {
		$from=$this->isValidMobile($from);
		if (!$from) $from="13234581949";
		$long_code=$this->query("select long_code from sms_long_codes where long_code='$from' or mobile='$from'")[0]['long_code'];
		$from=(empty($long_code) ? "13234581949" : $long_code);
